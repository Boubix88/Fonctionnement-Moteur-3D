...
// On génère une depthmap pour les ombres en utilisant le buffer z
void depthmap_triangle(Vec3f *pts, float *zbuffer) {
    Vec2i boxMin, boxMax;
    ...
    for (P.x = boxMin.x; P.x <= boxMax.x; P.x++) {
        for (P.y = boxMin.y; P.y <= boxMax.y; P.y++) {
            Vec3f bc_screen = calculBarycentrique(pts[0], pts[1], pts[2], P);
            if (bc_screen.x < 0 || bc_screen.y < 0 || bc_screen.z < 0) continue;
            P.z = 0;
            for (int i = 0; i < 3; i++) P.z += pts[i].z * bc_screen[i];
            if (zbuffer[int(P.x+P.y*width)] < P.z) {
                zbuffer[int(P.x+P.y*width)] = P.z;
                depthmap.set(P.x, P.y, TGAColor(255, 255, 255, 255));
            }
        }
    }
}
 
void triangle(Vec3f *pts, Vec2f *tex_coords, float *zbuffer, TGAImage &image, TGAImage &texture, TGAImage &normale, Vec3f light_dir, float intensities[3]) { 
        ...
        for (P.y = boxMin.y; P.y <= boxMax.y; P.y++) {
            ...
            if (coordBarycentrique.x > 0 && coordBarycentrique.y > 0 && coordBarycentrique.z > 0) {
                ...
                if (zbuffer[int(P.x+P.y*width)] <= P.z) {
                    ...
                    // Convertion de la couleur en un vecteur normal
                    ...
                    // Après avoir calculé l'intensité de la lumière
                    float shadow = 0.3 + 0.7*(shadowbuffer[int(P.x+P.y*width)] >= P.z);
                    color.r *= intensity * shadow;
                    color.g *= intensity * shadow;
                    color.b *= intensity * shadow;
...
int main(int argc, char** argv) {
    ...
    // Texture
    TGAImage texture; 
    texture.read_tga_file("texture/diablo3_pose_diffuse.tga");

    // Normale
    TGAImage normale;
    normale.read_tga_file("texture/diablo3_pose_nm.tga");
    ...
    // Après avoir initialisé le zbuffer
    for (int i = 0; i < width * height; i++) {
    ...
    // Avant de dessiner les triangles
    /*Matrix lightView = lookat(light_dir, center, Vec3f(0,1,0));
    Matrix lightProjection = Matrix::identity(4);
    Matrix lightViewport = viewport(width/8, height/8, width*3/4, height*3/4);
    lightProjection[3][2] = -1.f/(light_dir-center).norm();
    Matrix lightMatrix = lightViewport*lightProjection*lightView;*/
    /*for (int i = 0; i < model->nfaces(); i++) {
        Vec3f world_coords[3];
        for (int j = 0; j < 3; j++) {
            world_coords[j] = model->vert(face[j]);
        }
        Vec3f screen_coords[3] = { Vec3f(lightMatrix*Matrix(world_coords[0])), Vec3f(lightMatrix*Matrix(world_coords[1])), Vec3f(lightMatrix*Matrix(world_coords[2])) };
        depthmap_triangle(screen_coords, shadowbuffer);
    }*/
	...
	for (int i = 0; i < model->nfaces(); i++) {
        ...
        triangle(screen_coords, tex_coords, zbuffer, image, texture, normale, light_dir, intensities);
    }
